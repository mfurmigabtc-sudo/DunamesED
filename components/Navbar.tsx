    ...
    >
      <div className="container mx-auto px-4 md:px-8 flex justify-between items-center">
        ...
        <a 
          ...
          className="flex items-center gap-2 font-bold text-xl md:text-2xl text-slate-900"
        >
          <div className={`p-2 rounded-lg ${isScrolled ? 'bg-amber-500 text-white' : 'bg-white text-slate-900'}`}>
            <Hammer size={24} />
          </div>
          <span className={isScrolled ? 'text-slate-900' : 'text-white drop-shadow-md'}>
            Dunames<span className="text-amber-500">Edificações</span>
          </span>
        </a>
        ...
        <div className="hidden xl:flex items-center gap-6">
          {navLinks.map((link) => (
            <a
              ...
              className={`font-medium transition-colors hover:text-amber-500 text-sm xl:text-base ${
                isScrolled ? 'text-slate-700' : 'text-white shadow-black drop-shadow-md'
              }`}
            >
              {link.name}
            </a>
